...
class WechatController extends CommonController
{
    ...
    public function actionMessage()
    {
        ...
        $server->setMessageHandler(function ($message) {
            ...
            if ('event' == $message['MsgType']) { // 事件
                switch ($message['Event']) {
                    case 'SCAN': // 扫码
                        if (isset($message['EventKey']) && !empty($message['EventKey'])) { // 获取用户信息
                            $openId = $message['FromUserName'];
                            $conf = \Yii::$app->params['wx']['mp'];
                            $user = (new Application(['conf' => $conf]))->driver('mp.user');
                            $userInfo = $user->info($openId);

                            //更新用户数据
                            $userId = str_replace(ConStatus::$BIND_WECHAT, '', $message['EventKey']);
                            $managerInfo = User::findOne($userId);
                            if ($managerInfo) {
                                $managerInfo->wechat_name = $userInfo['nickname'];
                                $managerInfo->wechat_img = $userInfo['headimgurl'];
                                $managerInfo->wechat_openid = $userInfo['openid'];
                                $managerInfo->save();

                                $redis = \Yii::$app->redis;
                                $redis->set($message['EventKey'], json_encode($userInfo));
